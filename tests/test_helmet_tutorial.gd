...
func test_player_can_reach_and_push_mirror_without_passing_treasure_walls() -> void:
	...
	var pull_below_screen := world.pull_front(Vector2i.DOWN)
	assert_true(not pull_below_screen.success, "player cannot pull 镜 through the bottom screen boundary")
	assert_equal(world.player_pos, Vector2i(13, 17), "failed bottom pull keeps player inside the screen")
	assert_any_text(world, Vector2i(13, 16), "镜", "failed bottom pull keeps 镜 inside the screen")

	world.player_pos = Vector2i(13, 15)
	world.facing = Vector2i.DOWN
	var push_mirror_to_bottom := world.try_move_player(Vector2i.DOWN)
	assert_true(push_mirror_to_bottom.success, "player can push 镜 down to the last row")
	assert_any_text(world, Vector2i(13, 17), "镜", "镜 can sit on the last visible row")
	var push_below_screen := world.try_move_player(Vector2i.DOWN)
	assert_true(not push_below_screen.success, "player cannot push 镜 below the bottom screen boundary")
	assert_any_text(world, Vector2i(13, 17), "镜", "failed bottom push keeps 镜 on the last visible row")
...
func test_he_splits_first_then_context_text_spawns_later() -> void:
	...
	assert_no_text_anywhere(world, "呢", "no stale 呢 remains anywhere after 他 splits")
	assert_no_text_anywhere(world, "？", "no stale question mark remains anywhere after 他 splits")
	assert_no_text_anywhere(world, "」", "no stale quote remains anywhere after 他 splits")
...
func test_human_stacks_under_ye_and_restores_he() -> void:
	...
	assert_true(not world.get_any_entity_at(Vector2i(17, 14)).pushable, "诗 in final text is not pushable")
	assert_true(not world.get_any_entity_at(Vector2i(18, 14)).pushable, "人 in final text is not pushable")
