...
func _init() -> void:
	...
	if sword_gate == null:
		failures.append("interacting with the sword gate keeps the opened door entity")
	...
	world.facing = Vector2i.RIGHT
	var hand_transport := world.interact_front()
	if not hand_transport.success:
		failures.append("interacting with the hand gate is available")
	if world.pending_scene_path != ArtifactHall.GLOVE_SCENE_PATH:
		failures.append("hand gate interaction requests the glove scene")
	world.pending_scene_path = ""
