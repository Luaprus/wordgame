...
func play_at(origin: Vector2, cell_size: float, effect_duration := 0.42, seed_value := 2371) -> void:
	...
	rng.seed = seed_value
	for i in range(32):
		var angle: float = TAU * float(i) / 32.0 + rng.randf_range(-0.18, 0.18)
		var speed: float = rng.randf_range(cell_size * 0.55, cell_size * 1.5)
		particles.append({
...
func _draw() -> void:
	for particle: Dictionary in particles:
		var particle_delay: float = particle.delay
		var age: float = elapsed - particle_delay
		if age <= 0.0:
			continue
		var particle_life: float = particle.life
		var progress: float = clampf(age / particle_life, 0.0, 1.0)
