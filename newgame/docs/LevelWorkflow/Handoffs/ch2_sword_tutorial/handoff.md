...
## 2026-07-16 恢复正式剑关卡入口

`05_聖劍寶庫_復刻.tscn` 已恢复为正式流程入口：

- `start_at_snake_second_phase = false`
- 从迷宫进入宝库房间
- 完成找剑和圣剑获取动画
- 继续圣剑删除字教学、史莱姆段和再战蛇妖流程

`ReferenceSwordFlow.gd` 中的 `_enter_snake_second_phase_direct()` 仍保留给调试使用，但不再由正式场景入口自动调用。

## 2026-07-15 接手提醒：蛇妖段待修问题

以下问题本次先记录，方便后续对话直接接手，不要误以为是已验收效果：

1. 蛇妖部分，“我”字移动到最上方时会弹出“岩壁挡住了去路”。这个提示不符合当前蛇妖追逐段语境，后续需要去掉或改为不提示，避免玩家以为是普通岩壁碰撞。
2. 蛇妖进入二阶段后，“我”字位置和蛇妖对白会随着场景滚动突然发生变化。后续需要重点检查 `snake_scroll_offset`、`snake_player_scroll_remainder`、`_update_player_position()`、`_sync_snake_sentence_positions()` 和二阶段切换时的坐标重置。

相关实现集中在 `res://scripts/ReferenceSwordFlow.gd` 的蛇妖段，包括蛇妖滚屏、二阶段切换、反制句子显示和玩家平滑移动逻辑。
...
## 2026-07-16 史莱姆“史”字移动效果

本次按源项目 `D:/文字游戏/Scenes/Animations/slime_move.tres`，并结合 `L:/wordgame-map/wordgame/Scenes/Test/SlimeMoveDemo.tscn`，把“史”字的位移关键帧接入到 `ReferenceSwordFlow.gd` 的史莱姆段。

- 参考场景：`L:/wordgame-map/wordgame/Scenes/Test/SlimeMoveDemo.tscn`。
- 源动画：`D:/文字游戏/Scenes/Animations/slime_move.tres`，长度 `0.35s`，位移关键帧为 `0 → -30px → -30px → 0`，缩放关键帧为 `Vector2(1.2, 0.85)` 后回到 `Vector2(1, 1)`。
- `ReferenceSwordFlow.gd` 通过 `_sample_slime_move_offset_y()` 和 `_sample_slime_move_scale()` 复用源关键帧；循环只作用于可见“史”字。
- 史莱姆跑离时停用循环视觉采样，继续使用原有向右离场 Tween。
