...
static func _text_layout() -> Array:
	...
	# The upper-right cage, recorded cell by cell from the reference grid.
	_add(entries, "框", [
	...
	# The lower figure, also stored as individual grid cells so its colour layout stays fixed.
