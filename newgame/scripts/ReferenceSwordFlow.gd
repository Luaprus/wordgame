...
const DIALOGUE_DEFAULT_START_Y := 14
const SLIME_LEFT_INTRO_DIALOGUE_START_Y := 7
const SLIME_LEFT_FISSURE_OPEN_DIALOGUE_START_Y := 8
...
var active_dialogue_start_y := DIALOGUE_DEFAULT_START_Y
...
var snake_dynamic_map_labels: Array[Node] = []
...
func _start_slime_left_intro() -> void:
	...
	_start_dialogue(MAP_SLIME_LEFT, [
		["睁开眼睛……", "坚硬的岩壁围绕着四周。"]
	], Callable(self, "_show_left_fissure_sentence"), SLIME_LEFT_INTRO_DIALOGUE_START_Y)
...
func _left_fissure_deleted() -> void:
	...
	_start_dialogue(MAP_SLIME_LEFT, [
		["岩壁之间发出碎裂声。", "一条细缝露了出来。"]
	], Callable(self, "_unlock_left_cave"), SLIME_LEFT_FISSURE_OPEN_DIALOGUE_START_Y)
...
func _build_snake_dynamic_map() -> void:
	...
	for y in range(rows.size()):
		...
		for x in range(row.length()):
			...
			if text == " " or text == "＿" or text == "我":
				continue
			var node := _make_snake_dynamic_map_node(text, x, y, WALL_COLOR)
			root.add_child(node)
			snake_dynamic_map_labels.append(node)
	...
	for copy_index in range(SNAKE_LOOP_COPIES):
		for y in range(SNAKE_LOOP_SOURCE_ROWS.size()):
			...
			for x in range(row.length()):
				...
				var node := _make_snake_dynamic_map_node(text, x, absolute_y, WALL_COLOR)
				root.add_child(node)
				snake_dynamic_map_labels.append(node)


func _make_snake_dynamic_map_node(text: String, grid_x: int, grid_y: int, color: Color) -> Node:
	if text == "树":
		var tree_sprite := TreeSpriteScene.instantiate()
		tree_sprite.name = "TreeSprite"
		tree_sprite.position = Vector2(grid_x * CELL, grid_y * CELL)
		tree_sprite.set_meta("snake_keyword", text)
		return tree_sprite
	var label := _make_cell_label(text, grid_x, grid_y, color)
	label.set_meta("snake_keyword", text)
	return label
...
func _set_snake_keyword_color(keyword: String, color: Color) -> void:
	...
	for child in map_roots[MAP_SNAKE].get_children():
		if child is CanvasItem and String(child.get_meta("snake_keyword", "")) == keyword:
...
func _set_snake_map_dimmed(dimmed: bool) -> void:
	...
	for child in map_roots[MAP_SNAKE].get_children():
		if child is CanvasItem:
			var canvas_item := child as CanvasItem
			canvas_item.modulate = DIM_COLOR if dimmed else Color.WHITE
		if child is Label:
			var label := child as Label
...
func _start_dialogue(map_index: int, pages: Array, callback: Callable = Callable(), start_y := DIALOGUE_DEFAULT_START_Y) -> void:
	...
	active_dialogue_start_y = start_y
...
func _advance_dialogue() -> void:
	...
	if dialogue_index < dialogue_pages.size():
		...
		return
	dialogue_active = false
	active_dialogue_start_y = DIALOGUE_DEFAULT_START_Y
	_clear_dialogue()
...
func _render_dialogue(map_index: int, lines: Array, with_continue: bool) -> void:
	...
	for line_index in range(lines.size()):
		...
		if with_continue and line_index == lines.size() - 1:
			line += "▽"
		var start_x: int = 6
		var y: int = active_dialogue_start_y + line_index
		if map_index == MAP_SNAKE:
...
func _play_backspace_cut_animation(label: Label) -> void:
	...
	var original_visible := label.visible
	...
	var word_viewport := _build_backspace_word_viewport(label)
	actor_layer.add_child(word_viewport)
	await get_tree().process_frame

	var material := ShaderMaterial.new()
	material.shader = BACKSPACE_CUT_SHADER
	material.set_shader_parameter("degree", BACKSPACE_CUT_ANGLE_DEGREES)
	...
	split_sprite.position = label.position + label.size * 0.5
	split_sprite.z_index = 72
	split_sprite.modulate = DIALOGUE_COLOR
	actor_layer.add_child(split_sprite)
	label.visible = false
	...
	slash.rotation_degrees = BACKSPACE_CUT_ANGLE_DEGREES
	...
	await get_tree().create_timer(0.08).timeout
	split_sprite.modulate.a = 0.0
	label.visible = original_visible
...
func _build_backspace_word_viewport(source_label: Label) -> SubViewport:
	var viewport := SubViewport.new()
	viewport.name = "BackspaceWordViewport"
	viewport.size = Vector2i(int(source_label.size.x), int(source_label.size.y))
	viewport.transparent_bg = true
	viewport.render_target_update_mode = SubViewport.UPDATE_ALWAYS

	var rendered_label := _make_label(source_label.text, 56, DIALOGUE_COLOR)
	rendered_label.position = Vector2.ZERO
	rendered_label.size = source_label.size
...
func _build_direction_marker() -> void:
	...
	direction_marker.z_index = 4
	player_label.add_child(direction_marker)
...
func _show_direction_marker(direction: Vector2i) -> void:
	if direction == Vector2i.ZERO or direction_marker == null:
		return
...
func _sync_direction_marker() -> void:
	if direction_marker == null or direction_marker_direction == Vector2i.ZERO:
		return
	var points := PlayerDirectionMarker.local_points(CELL, direction_marker_direction)
	direction_marker.position = Vector2(CELL * 0.5, CELL * 0.5) + PlayerDirectionMarker.anchor_offset(CELL, direction_marker_direction)
