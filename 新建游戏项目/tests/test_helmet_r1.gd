...
func test_bridge_merge_and_split_effects() -> void:
	...
	assert_any_text(world, Vector2i(20, 9), "桥", "upper bridge rail starts on the left bank")
	assert_any_text(world, Vector2i(21, 9), "桥", "upper bridge rail replaces creek")
	assert_true(world.get_any_entity_at(Vector2i(21, 9)).solid, "bridge glyph itself blocks movement")
	assert_true(world.get_any_entity_at(Vector2i(21, 10)) == null, "middle bridge row is an empty road")
	assert_any_text(world, Vector2i(21, 11), "桥", "lower bridge rail replaces creek")
	assert_true(world.get_any_entity_at(Vector2i(19, 8)).text == "桥", "tree top becomes bridge")
	...
	assert_any_text(world, Vector2i(21, 9), "溪", "creek returns after bridge split")
	assert_any_text(world, Vector2i(21, 10), "溪", "middle road returns to creek after bridge split")
	assert_any_text(world, Vector2i(21, 11), "溪", "lower rail returns to creek after bridge split")
